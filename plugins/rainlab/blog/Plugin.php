...
use Event;
use Backend\Models\BackendBackup;
...
class Plugin extends PluginBase
{
    ...
    public function registerComponents()
    {
        return [
            ...
            'RainLab\Blog\Components\Tags'       => 'blogTags',
            'RainLab\Blog\Components\Menus'       => 'blogMenu',
        ];
    }
...
//            ]
            ...
            'blog' => [
                ...
                'sideMenu' => [
                    'new_post' => [
                        ...
                        'permissions' => ['backend.blog.add_new_post']
                    ],
                    'posts' => [
    ...
    public function registerSchedule($schedule)
    {
        $schedule->call(function () {
            app('backend.notify')->resetNotifyRequestApprove();
            app('backend.notify')->resetNotifyRejectPost();
        })->daily();

        $schedule->call(function () {
            app('backend.approve')->updatePublicPostSetTime();
        })->everyMinute();


        if (Schema::hasTable('site_backup_setting')) {
            $backup = BackendBackup::first();
        } else {
            $backup = null;
        }

        if ($backup) {
            if ($backup->schedule_option == 1) {
                $time = $backup->schedule_time_1 ? "0 $backup->schedule_time_1 * * *" : "0 0 * * *";

                $schedule->call(function () {
                    app('backend.backup')->backupMedia();
                    app('backend.backup')->dumpSql();
                })->cron($time);

            } elseif ($backup->schedule_option == 2) {
                $time = $backup->schedule_time_2 ? "0 $backup->schedule_time_2 * * $backup->schedule_day" : "0 0 * * $backup->schedule_day";

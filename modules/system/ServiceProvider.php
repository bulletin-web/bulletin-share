...
class ServiceProvider extends ModuleServiceProvider
{
    ...
    protected function registerSingletons()
    {
        ...
        App::singleton('backend.info', function () {
            return new \Backend\Classes\InfoController;
        });

        App::singleton('display.setting', function () {
            return new \Backend\Classes\DisplaySettingController;
        });

        App::singleton('content.tag', function () {
            return new \Backend\Classes\ContentTag;
        });

        App::singleton('content.menu', function () {
            return new \Backend\Classes\BackendMenu;
        });

        App::singleton('backend.approve', function () {
            return new \Backend\Classes\BackendApprove;
        });

        App::singleton('backend.authenticate', function () {
            return new \Backend\Classes\Authenticate;
        });

        App::singleton('cms.post', function () {
            return new \Backend\Classes\Posts;
        });
    }
...
//                ],
                'system.manage_mail_settings' => [
                    'label' => 'system::lang.permissions.manage_mail.mail_setting',
                    'tab' => 'system::lang.permissions.tab.manage_mail',
                    'order' => 80
                ],
                'system.manage_mail_templates' => [
                    'label' => 'system::lang.permissions.manage_mail.mail_template',
                    'tab' => 'system::lang.permissions.tab.manage_mail',
                    'order' => 90
                ]
            ]);
    ...
    protected function registerBackendSettings()
    {
        ...
        SettingsManager::instance()->registerCallback(function ($manager) {
            $manager->registerSettingItems('October.System', [
                'site_infos' => [
                    'label'       => 'サイト基本情報',
                    'category'    => SettingsManager::CATEGORY_WEBSITE,
                    'url'         => Backend::url('backend/info'),
                    'permissions' => ['backend.site_infos'],
                    'order'       => 90
                ],
                'site_display' => [
                    'label'       => 'サイト表示',
                    'category'    => SettingsManager::CATEGORY_WEBSITE,
                    'url'         => Backend::url('backend/displaysetting'),
                    'permissions' => ['backend.site_display_setting'],
                    'order'       => 100
                ],
                'site_backup' => [
                    'label'       => 'バックアップ・復元',
                    'category'    => SettingsManager::CATEGORY_WEBSITE,
                    'url'         => Backend::url('backend/backup'),
                    'permissions' => ['backend.access_backup'],
                    'order'       => 100
                ],
                'user_groups' => [
                    'label'       => 'backend::lang.user.groups',
                    'category'    => SettingsManager::CATEGORY_ADMINISTRATORS,
                    'url'         => Backend::url('backend/usergroups'),
                    'permissions' => ['backend.group.*'],
                    'order'       => 110
                ],
                'users' => [
                ...
                'user_roles' => [
                    ...
                    'permissions' => ['backend.role.*'],
                    'order'       => 110
                ],
                'workflow' => [
                    'label'       => 'ワークフロー設定',
                    'category'    => SettingsManager::CATEGORY_TODOS,
                    'url'         => Backend::url('backend/workflow'),
                    'permissions' => ['backend.workflow_setting'],
                    'order'       => 120
                ],
                'sitebar' => [
                    'label'       => 'サイドバーウィジット管理',
                    'category'    => SettingsManager::PLUGIN_MANAGER,
                    'url'         => Backend::url('backend/sitebar'),
                    'order'       => 120
                ],
//                'content_0' => [
//                    'label'       => 'テンプレート',
//                    'category'    => SettingsManager::CATEGORY_CONTENTS,
//                    'url'         => Backend::url('#'),
...
//                ],
                'backend_menu' => [
                    'label'       => 'メニュー',
                    'category'    => SettingsManager::CATEGORY_CONTENTS,
                    'url'         => Backend::url('backend/menu'),
                    'permissions' => ['backend.blog.menu_setting'],
                    'order'       => 120
                ],
                'blog_category' => [
                    'label'       => 'カテゴリー',
                    ...
                    'permissions' => ['backend.blog.edit_category'],
                    'order'       => 120
                ],
                'blog_tags' => [
                    'label'       => 'タグ',
                    'category'    => SettingsManager::CATEGORY_CONTENTS,
                    'url'         => Backend::url('rainlab/blog/tag'),
                ...
                'comment_blacklist' => [
                    ...
                    'category'    => SettingsManager::CATEGORY_COMMENTS,
                    'url'         => Backend::url('backend/commentblacklist'),
                    'permissions' => ['backend.comment.access_blacklist'],
                    'order'       => 140
                ],
//                'updates' => [

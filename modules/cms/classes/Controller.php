...
class Controller
{
    ...
    {
        app('backend.authenticate')->limitAccess();
        app('backend.authenticate')->checkAuth();

        /*
         * Check Maintain
         */
        if (isset(Info::first()->status) && Info::first()->status ==2)
        {
            return Response::make(View::make('cms::maintain'));
        }

        if (app('backend.authenticate')->checkBasicAuth()) {
            return Backend::redirect('backend/auth/login');
        }
